...
namespace confighttp {
  ...
  constexpr auto SESSION_EXPIRE_DURATION = 24h * 15;  ///< Lifetime of a Web UI login session.
  ...
  /**
   * @brief Authenticate the request by login session cookie or HTTP Basic credentials.
   *
   * @param response HTTP response object to populate on failure.
   * @param request HTTP request data from the client.
   * @param needsRedirect Redirect to the login page on failure instead of answering 401 (for HTML pages).

...
namespace config {
  ...
  struct audio_t {
    ...
    bool keep_default;  ///< Keep the system default audio sink unchanged while streaming.
    bool auto_capture;  ///< Automatically select the currently active audio sink for capture.
  };
  ...
  struct input_t {
    ...
    bool enable_input_only_mode;  ///< Allow clients to connect in input-only mode without a video stream.
    bool forward_rumble;  ///< Forward controller rumble events to the client.
  };
  ...
  /**
   * @brief Server-side command that clients holding the server-command permission may trigger.
   */
  struct server_cmd_t {
    ...
    bool elevated;  ///< Whether the command should be launched elevated.
  };
   ...
   */
  struct sunshine_t {
    bool hide_tray_controls;  ///< Hide the control entries from the system tray menu.
    bool enable_pairing;  ///< Allow new clients to pair with this host.
    bool enable_discovery;  ///< Advertise this host via mDNS discovery.
    bool envvar_compatibility_mode;  ///< Enable environment-variable compatibility mode for scripts written against Sunshine.
    ...
    bool legacy_ordering;  ///< Use the legacy ordering for the application list.
    ...
    std::vector<prep_cmd_t> state_cmds;  ///< Global state commands executed when a stream starts and stops.
    std::vector<server_cmd_t> server_cmds;  ///< Server commands that clients can trigger remotely.

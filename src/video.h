...
namespace video {
  ...
  struct config_t {
    ...
    // (The probe configs in video.cpp use positional initialization for the upstream fields.)
    ...
    int encodingFramerate;  ///< Requested display framerate in mHz (Apollo).
    bool input_only;  ///< Whether this is an input-only session with no video (Apollo).
  };

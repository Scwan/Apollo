...
export default {
  ...
  methods: {
    logout() {
      // Apollo's login page stores the web session in the "auth" cookie (see /api/login in
      // confighttp.cpp); drop it so the next request lands on the login page again.
      document.cookie = 'auth=; Max-Age=0; Path=/; Secure; SameSite=Strict'

      // Also clear any cached HTTP basic-auth credentials the browser may still hold.

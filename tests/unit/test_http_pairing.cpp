...
TEST_P(PairingTest, Run) {
  ...
  // phase 4
  // Apollo adds the paired client straight to the authorized-client list instead of handing the
  // certificate back through a queue, so check the client list and keep the state off the disk.
  const auto input_client_name = input.session->client.name;
  ...
  // Check that we actually added the input client to the authorized-client list
  const auto clients = get_all_clients();
  if (expected.phase_4_success) {
    ASSERT_EQ(clients.size(), clients_before + 1);
    ASSERT_EQ(clients.back().at("name").get<std::string>(), input_client_name);
  } else {
    ASSERT_EQ(clients.size(), clients_before);
  }
}
